...
|---|---|---|
| `scripts/nightly-tcsb-revision-sync.py` | Canonical Bookish Lamp → BibleStudyMan TCSB text sync. | Copies BL verses, generates BSM `versePlain`, rebuilds `bibleComplete.sql`, imports it into MariaDB, commits/pushes when enabled. Supports `local_paths.json` for non-sibling repo layouts. Use `--no-db-import` for dry/file-only runs. |
